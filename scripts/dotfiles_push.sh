...
No markdown. No code blocks. No bullet points in output. No extra commentary. No blank lines in the commit message, except the one between the subject line and the commit message body.'
...
if [[ -z "$GEMINI_API_KEY" ]]; then
  ...
  if [[ "$HTTP_CODE" -ne 200 ]]; then
    ...
    if [[ "${PRIMARY_ERROR_MSG,,}" == *"high demand"* ]]; then
      ...
      if can_notify; then
        notify-send -u normal -i github "Dotfiles" "Gemini-3.8-flash failed: $PRIMARY_ERROR_MSG. Retrying with gemini-3.5-flash."
      fi
  ...
  if [[ "$HTTP_CODE" -ne 200 ]]; then
    if can_notify; then
      if [[ "$USED_FALLBACK_MODEL" -eq 1 ]]; then
        ...
        notify-send -u normal -i github "Dotfiles" "Gemini-3.5-flash failed: $ERROR_MSG. Falling back."
      else
